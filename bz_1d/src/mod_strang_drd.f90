...
subroutine strang_integration_drd(f_loc_reac, f_diff, tol, neq, nx, u, nt, tini, tend)
  implicit none
  ! arguments
  interface
  ...
  do it = 1, nt-1
    ...
    !!print *, " Rock4 integration for diffusion on dt/2"
    call rock4_integration(f_diff, tol, ntot, u, 1, t, t+(dt/2), .false.)
    ...
    !!print *, " Rock4 integration for diffusion on dt/2"
    call rock4_integration(f_diff, tol, ntot, u, 1, t+(dt/2), t+dt, .false.)
   

  end do

end subroutine strang_integration_drd

end module mod_strang_drd
